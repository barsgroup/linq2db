...
{
    internal class GroupByBuilder : MethodCallBuilder
    {
        #region KeyContext
        ...
        {
            private readonly SelectContext _element;
            ...
            private readonly Type _groupingType;
            private readonly KeyContext _key;

            private readonly Expression _sequenceExpr;
            ...
            private Expression BuildGrouping()
            {
            ...
            {
                private readonly Func<IDataContext, TKey, object[], IQueryable<TElement>> _itemReader;
                private readonly List<ParameterAccessor> _parameters;
                ...
                private List<TElement> GetItems()
                {
            ...
            private class GroupByHelper<TKey, TElement, TSource> : IGroupByHelper
            {
...
// ReSharper restore AssignNullToNotNullAttribute
                ...
                private static IGrouping<TKey, TElement> GetGrouping(
            ...
            private class ContextHelper<T> : IContextHelper
            {
                public Expression GetContext(Expression sequence, ParameterExpression param, Expression expr1,
                    Expression expr2)
                {
// ReSharper disable AssignNullToNotNullAttribute
...
// ReSharper restore AssignNullToNotNullAttribute
                    var arg2 = Expression.Lambda<Func<T, bool>>(Expression.Equal(expr1, expr2), param);
            ...
            if (body.NodeType == ExpressionType.MemberInit)
            {
                ...
                if (throwExpr)
                    throw new NotSupportedException(
                        "Explicit construction of entity type '{0}' in group by is not allowed.".Args(body.Type));
            }
