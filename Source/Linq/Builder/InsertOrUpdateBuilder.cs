...
{
    internal class InsertOrUpdateBuilder : MethodCallBuilder
    {
        #region UpdateContext

        private class InsertOrUpdateContext : SequenceContextBase
        {
            ...
            BuildInfo buildInfo)
        {
            ...
            if (methodCall.Arguments.Count == 3)
            {
                ...
                if (keys.Count == 0)
                    throw new LinqException("InsertOrUpdate method requires the '{0}' table to have a primary key.",
                        table.Name);
