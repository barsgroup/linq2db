...
namespace Bars2Db.Linq.Builder
{
    public partial class ExpressionBuilder
    {
        ...
        private static List<ISequenceBuilder> _sequenceBuilders = new List<ISequenceBuilder>
        {
            new TableBuilder(),
            new SelectBuilder(),
        ...
        private readonly List<ISequenceBuilder> _builders = _sequenceBuilders;
        private bool _reorder;
        private readonly Dictionary<Expression, Expression> _expressionAccessors;
        ...
        private readonly HashSet<Expression> _visitedExpressions;
            ...
            ParameterExpression[] compiledParameters)
        {
            ...
            {
                DataReaderLocal =
                    BuildVariable(Expression.Convert(DataReaderParam, dataContext.DataContext.DataReaderType), "ldr");
            }
        }
        ...
        private Expression ConvertExpressionTree(Expression expression)
        {
            ...
            var sequence = ConvertSequence(new BuildInfo((IBuildContext) null, expr, new SelectQuery()),
                SequenceParameter);
            ...
            if (sequence != null)
            {
                if (sequence.Expression.Type != expr.Type)
                {
                    ...
                    {
                        var p =
                            sequence.ExpressionsToReplace.SingleOrDefault(
                                s => s.Path.NodeType == ExpressionType.Parameter);
                                ...
                                return
                                    Expression.Convert(
                                        Expression.ArrayIndex(
                                            ParametersParam,
                                            Expression.Constant(Array.IndexOf(CompiledParameters,
        ...
        private Expression ExposeExpression(Expression expression)
        {
            return expression.Transform(expr =>
            {
                switch (expr.NodeType)
                {
                    case ExpressionType.MemberAccess:
                    {
                        ...
                        if (l != null)
                        {
                            ...
                            var ex =
                                body.Transform(wpi => wpi.NodeType == ExpressionType.Parameter ? me.Expression : wpi);
        ...
        public MethodInfo[] EnumerableMethods
            => _enumerableMethods ?? (_enumerableMethods = typeof(Enumerable).GetMethodsEx());

        private MethodInfo[] _queryableMethods;

        public MethodInfo[] QueryableMethods
            => _queryableMethods ?? (_queryableMethods = typeof(Queryable).GetMethodsEx());

        private readonly Dictionary<Expression, Expression> _optimizedExpressions =
        ...
        private Expression OptimizeExpression(Expression expression)
        {
            ...
            _optimizedExpressions[expression] =
                expr = expression.Transform((Func<Expression, TransformInfo>) OptimizeExpressionImpl);

            return expr;
        }

        private TransformInfo OptimizeExpressionImpl(Expression expr)
        {
            switch (expr.NodeType)
            {
                case ExpressionType.MemberAccess:
                ...
                case ExpressionType.Call:
                {
                    ...
                    if (call.IsQueryable())
                    {
                        ...
                        {
                            case "Where":
                                return new TransformInfo(ConvertWhere(call));
                            case "GroupBy":
                                return new TransformInfo(ConvertGroupBy(call));
                            case "SelectMany":
                                return new TransformInfo(ConvertSelectMany(call));
                            case "Select":
                                return new TransformInfo(ConvertSelect(call));
                            case "LongCount":
                            case "Count":
                            case "Single":
                            ...
                            case "First":
                            case "FirstOrDefault":
                                return new TransformInfo(ConvertPredicate(call));
                            case "Min":
                            case "Max":
                                return new TransformInfo(ConvertSelector(call, true));
                            case "Sum":
                            case "Average":
                                return new TransformInfo(ConvertSelector(call, false));
                            case "ElementAt":
                            case "ElementAtOrDefault":
                                return new TransformInfo(ConvertElementAt(call));
                            case "LoadWith":
        ...
        private Expression ConvertSingleOrFirst(Expression expr, MethodCallExpression call)
        {
            ...
            var method = GetQueriableMethodInfo(call,
                (m, _) => m.Name == call.Method.Name && m.GetParameters().Length == 1);
            var select = call.Method.DeclaringType == typeof(Enumerable)
                ...
                : QueryableMethods
                    ...
                    .First(
                        m =>
                            m.GetParameters()[1].ParameterType.GetGenericArgumentsEx()[0].GetGenericArgumentsEx().Length ==
                            2);
            ...
            lbody.Visit(ex =>
            {
                if (ex.NodeType == ExpressionType.Call)
                {
                    ...
                    if (call.Arguments.Count > 0)
                    {
                        ...
                        {
                            while (arg.NodeType == ExpressionType.Call &&
                                   ((MethodCallExpression) arg).Method.Name == "Select")
            ...
            if (!ReferenceEquals(sequence, method.Arguments[0]) || !ReferenceEquals(predicate, method.Arguments[1]))
            {
                ...
                if (exprs.Count > 0)
                {
                    ...
                    method = Expression.Call(methodInfo, method,
                        Expression.Lambda(
                            exprs.Aggregate((Expression) parameter,
                                (current, _) => Expression.PropertyOrField(current, "p")),
                            parameter));
                }
            }
        ...
        private interface IGroupByHelper
        {
            void Set(bool wrapInSubQuery, Expression sourceExpression, LambdaExpression keySelector,
        ...
        private class GroupByHelper<TSource, TKey, TElement, TResult> : IGroupByHelper
        {
            ...
            {
                Expression<Func<IQueryable<TSource>, TKey, TElement, TResult, IQueryable<IGrouping<TKey, TSource>>>>
                    func = (source, key, e, r) => source
            ...
            {
                Expression<Func<IQueryable<TSource>, TKey, TElement, TResult, IQueryable<TResult>>> func =
                    (source, key, e, r) => source
            ...
            {
                Expression<Func<IEnumerable<TSource>, TKey, TElement, TResult, IEnumerable<TResult>>> func =
                    (source, key, e, r) => source
                        .GroupBy(keyParam => key, elemParam => e)
                        .Select(resParam => r)
                    ;
            ...
            {
                Expression<Func<IQueryable<TSource>, TKey, TElement, TResult, IQueryable<IGrouping<TKey, TElement>>>>
                    func = (source, key, e, r) => source
            ...
            {
                Expression<Func<IEnumerable<TSource>, TKey, TElement, TResult, IEnumerable<IGrouping<TKey, TElement>>>>
                    func = (source, key, e, r) => source
                        .Select(selectParam => new GroupSubQuery<TKey, TSource>
                        {
                            Key = key,
                ...
                ParameterExpression resArg)
            {
                ...
                var expr = body.Transform(ex =>
                {
                    ...
                    if (ReferenceEquals(ex, func.Parameters[1]))
                        return
                            _keySelector.Body.Transform(e => ReferenceEquals(e, _keySelector.Parameters[0]) ? keyArg : e);
                    ...
                    if (ReferenceEquals(ex, func.Parameters[2]))
                    {
                        ...
                        return
                            _elementSelector.Body.Transform(
                                e => ReferenceEquals(e, _elementSelector.Parameters[0]) ? obj : e);
                    }
        ...
        private static LambdaExpression GetLambda(Expression expression, params int[] n)
        {
            foreach (var i in n)
        ...
        private Expression ConvertGroupBy(MethodCallExpression method)
        {
            ...
            var elementSelector = types.ContainsKey("TElement")
                ? (LambdaExpression) OptimizeExpression(method.Arguments[2].Unwrap())
                : null;
            var resultSelector = types.ContainsKey("TResult")
                ? (LambdaExpression) OptimizeExpression(method.Arguments[types.ContainsKey("TElement") ? 3 : 2].Unwrap())
                : null;
        ...
        private interface ISelectManyHelper
        {
        ...
        private class SelectManyHelper<TSource, TCollection> : ISelectManyHelper
        {
            private LambdaExpression _colSelector;
            private Expression _sourceExpression;
            ...
            public Expression AddElementSelectorQ()
            {
                Expression<Func<IQueryable<TSource>, IEnumerable<TCollection>, IQueryable<TCollection>>> func =
                    (source, col) => source
            ...
            public Expression AddElementSelectorE()
            {
                Expression<Func<IEnumerable<TSource>, IEnumerable<TCollection>, IEnumerable<TCollection>>> func =
                    (source, col) => source
                        .SelectMany(colParam => col, (s, c) => c)
                    ;
            ...
            private Expression Convert(LambdaExpression func, ParameterExpression colArg)
            {
                var body = func.Body.Unwrap();
                var expr = body.Transform(ex =>
                {
        ...
        private Expression ConvertPredicate(MethodCallExpression method)
        {
            ...
            var cm = GetQueriableMethodInfo(method,
                (m, _) => m.Name == method.Method.Name && m.GetParameters().Length == 1);
        ...
        private Expression ConvertIQueriable(Expression expression)
        {
            if (expression.NodeType == ExpressionType.MemberAccess || expression.NodeType == ExpressionType.Call)
            {
                ...
                var l = Expression.Lambda<Func<Expression, IQueryable>>(Expression.Convert(expr, typeof(IQueryable)), p);
        ...
        private Expression ConvertElementAt(MethodCallExpression method)
        {
            ...
            var firstMethod = GetQueriableMethodInfo(method,
                (mi, _) => mi.Name == methodName && mi.GetParameters().Length == 1);
        ...
        private MethodInfo GetQueriableMethodInfo(MethodCallExpression method, Func<MethodInfo, bool, bool> predicate)
        {
            return method.Method.DeclaringType == typeof(Enumerable)
                ? EnumerableMethods.FirstOrDefault(m => predicate(m, false)) ??
                  EnumerableMethods.First(m => predicate(m, true))
                : QueryableMethods.FirstOrDefault(m => predicate(m, false)) ??
                  QueryableMethods.First(m => predicate(m, true));
        }
