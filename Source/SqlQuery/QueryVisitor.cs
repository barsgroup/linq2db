...
namespace LinqToDB.SqlQuery
{
        ...
        where TSearch : class
    {
        ...
        public readonly ParameterExpression ResultListParam = Expression.Parameter(typeof(LinkedList<TResult>), "resultList");
    ...
    public class ApplyWhileFalseStrategy<TSearch, TResult> : SearchStrategy<TSearch, TResult>
        where TSearch : class
    {
        public override Expression<ResultDelegate<TSearch, TResult>> GetStrategyExpression(Expression[] executeChildrenExpressions, bool isRoot)
        {
            ...
            var block = Expression.Block(new[] { castVariable, actionResultVariable }, castAssign, conditionalAddOrCallChildren, endOfFunctionLabel);
    ...
    public class QueryVisitor
    {
        ...
        public static void FindParentFirst(IQueryElement element, Func<IQueryElement, IQueryElement> action)
        {
            ...
            SearchEngine<IQueryElement>.Current.Find(element, resultList, new ApplyWhileFalseStrategy<IQueryElement, IQueryElement>(), visited, action);
        }
        ...
        private static void FindOnceInternal<TElementType>(IQueryElement element, LinkedList<TElementType> resultList, HashSet<object> visited)
        ...
        public static LinkedList<TElementType> FindDownTo<TElementType>(params IQueryElement[] elements) where TElementType : class, IQueryElement
        {
            var resultList = new LinkedList<TElementType>();
            var visited = new HashSet<object>();
        ...
        IQueryElement ConvertInternal(IQueryElement element, Func<IQueryElement, IQueryElement> action)
        {
            ...
            switch (element.ElementType)
            {
                ...
                case EQueryElementType.LikePredicate:
                {
                    ...
                    if (e1 != null && !ReferenceEquals(p.Expr1, e1) || e2 != null && !ReferenceEquals(p.Expr2, e2) || es != null && !ReferenceEquals(p.Escape, es))
                ...
                case EQueryElementType.BetweenPredicate:
                {
                    ...
                    if (e1 != null && !ReferenceEquals(p.Expr1, e1) || e2 != null && !ReferenceEquals(p.Expr2, e2) || e3 != null && !ReferenceEquals(p.Expr3, e3))
                ...
                case EQueryElementType.UpdateClause:
                {
                    ...
                    if (t != null && !ReferenceEquals(s.Table, t) || i != null && !ReferenceEquals(s.Items, i) || k != null && !ReferenceEquals(s.Keys, k))
                    {
                ...
                case EQueryElementType.SelectClause:
                {
                    ...
                    if (parent != null || cols != null && !ReferenceEquals(sc.Columns, cols) || take != null && !ReferenceEquals(sc.TakeValue, take) || skip != null && !ReferenceEquals(sc.SkipValue, skip))
                    {
                ...
                case EQueryElementType.SqlQuery:
                {
                    ...
                    nq.Init(ic, uc, dc, sc, fc, wc, gc, hc, oc, us, (ISelectQuery)parent, q.CreateTable, q.IsParameterDependent, ps);
        ...
        T[] Convert<T>(T[] arr, Func<IQueryElement, IQueryElement> action) where T : class, IQueryElement
        {
            return Convert(arr, action, null);
        }

        T[] Convert<T>(T[] arr1, Func<IQueryElement, IQueryElement> action, Clone<T> clone) where T : class, IQueryElement
        {
            T[] arr2 = null;
        ...
        LinkedList<T> Convert<T>(LinkedList<T> list, Func<IQueryElement, IQueryElement> action) where T : class, IQueryElement
        {
            return Convert(list, action, null);
        }

        LinkedList<T> Convert<T>(LinkedList<T> list1, Func<IQueryElement, IQueryElement> action, Clone<T> clone) where T : class, IQueryElement
        {
        ...
        List<T> Convert<T>(List<T> list, Func<IQueryElement, IQueryElement> action) where T : class, IQueryElement
        {
            return Convert(list, action, null);
        }

        List<T> Convert<T>(List<T> list1, Func<IQueryElement, IQueryElement> action, Clone<T> clone) where T : class, IQueryElement
        {
            List<T> list2 = null;
