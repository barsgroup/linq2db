...
namespace Bars2Db.SchemaProvider
{
    public abstract class SchemaProviderBase : ISchemaProvider
    {
        ...
        public virtual DatabaseSchema GetSchema(DataConnection dataConnection, GetSchemaOptions options = null)
        {
            ...
            if (options.GetTables)
            {
                ...
                foreach (var column in columns)
                {
                    ...
                    var systemType = GetSystemType(dataType, column.c.ColumnType, column.dt, column.c.Length,
                        column.c.Precision, column.c.Scale);
                    ...
                    column.t.Columns.Add(new ColumnSchema
                    {
                        ...
                        ColumnType =
                            column.c.ColumnType ??
                            GetDbType(dataType, column.dt, column.c.Length, column.c.Precision, column.c.Scale),
                ...
                foreach (var fk in fks.OrderBy(f => f.Ordinal))
                {
                    ...
                    var otherColumn =
                        (from c in otherTable.Columns where c.ColumnName == fk.OtherColumn select c).Single();
                            ...
                            select new ProcedureSchema
                            {
                                ...
                                Parameters =
                                    (
                                        ...
                                        let systemType =
                                            GetSystemType(pr.DataType, null, dt, pr.Length, pr.Precision, pr.Scale)
                                        orderby pr.Ordinal
        ...
        protected virtual DataTable GetProcedureSchema(DataConnection dataConnection, string commandText,
            CommandType commandType, DataParameter[] parameters)
        {
            using (
                var rd = dataConnection.ExecuteReader(commandText, commandType, CommandBehavior.SchemaOnly, parameters))
            {
        ...
        protected virtual Type GetSystemType(string dataType, string columnType, DataTypeInfo dataTypeInfo, long? length,
                ...
                if (!string.IsNullOrWhiteSpace(format) && !string.IsNullOrWhiteSpace(parms))
                {
                    ...
                    for (var i = 0; i < paramNames.Length; i++)
                    {
                        switch (paramNames[i].Trim().ToLower())
                        {
                            case "size":
                            case "length":
                                paramValues[i] = length;
                                break;
                            case "max length":
                                paramValues[i] = length == int.MaxValue
                                    ? "max"
                                    : length.HasValue ? length.ToString() : null;
                                break;
                            case "precision":
                                paramValues[i] = prec;
                                break;
                            case "scale":
                                paramValues[i] = scale.HasValue || paramNames.Length == 2 ? scale : prec;
                                break;
                        }
                    }
            ...
            foreach (var t in databaseSchema.Tables)
            {
                ...
                foreach (var key in t.ForeignKeys)
                {
                    ...
                    if (name == null)
                    {
                        ...
                        if (key.BackReference != null && key.ThisColumns.Count == 1 &&
                            key.ThisColumns[0].MemberName.ToLower().EndsWith("id"))
                        {
