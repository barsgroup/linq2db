    ...
    public class OracleSqlOptimizer : BasicSqlOptimizer
    {
        ...
        public override IQueryExpression ConvertExpression(IQueryExpression expr)
        {
            ...
            if (sqlBinaryExpression != null)
            {
                ...
                {
                    case "%":
                        return new SqlFunction(sqlBinaryExpression.SystemType, "MOD", sqlBinaryExpression.Expr1,
                            sqlBinaryExpression.Expr2);
                    case "&":
                        return new SqlFunction(sqlBinaryExpression.SystemType, "BITAND", sqlBinaryExpression.Expr1,
                            sqlBinaryExpression.Expr2);
                    case "|": // (a + b) - BITAND(a, b)
                        return Sub(
                            Add(sqlBinaryExpression.Expr1, sqlBinaryExpression.Expr2, sqlBinaryExpression.SystemType),
                            new SqlFunction(sqlBinaryExpression.SystemType, "BITAND", sqlBinaryExpression.Expr1,
                                sqlBinaryExpression.Expr2),
            ...
            else
            {
                ...
                if (sqlFunction != null)
                {
                    ...
                    {
                        case "Coalesce":
                            return new SqlFunction(sqlFunction.SystemType, "Nvl", sqlFunction.Parameters);
                        case "Convert":
                        {
                            ...
                            if (ftype == typeof(DateTime) || ftype == typeof(DateTimeOffset))
                            {
                                if (IsTimeDataType(sqlFunction.Parameters[0]))
                                {
                                    ...
                                    return new SqlFunction(sqlFunction.SystemType, "To_Char", sqlFunction.Parameters[1],
                                        new SqlValue("HH24:MI:SS"));
                                }

                                if (sqlFunction.Parameters[1].SystemType.ToUnderlying() == typeof(DateTime) &&
                                    IsDateDataType(sqlFunction.Parameters[0], "Date"))
                                {
                                    return new SqlFunction(sqlFunction.SystemType, "Trunc", sqlFunction.Parameters[1],
                                        new SqlValue("DD"));
                                }

                                return new SqlFunction(sqlFunction.SystemType, "To_Timestamp", sqlFunction.Parameters[1],
                                    new SqlValue("YYYY-MM-DD HH24:MI:SS"));
                            }
                        ...
                        case "CharIndex":
                            return sqlFunction.Parameters.Length == 2
                                ? new SqlFunction(sqlFunction.SystemType, "InStr", sqlFunction.Parameters[1],
                                    sqlFunction.Parameters[0])
                                : new SqlFunction(sqlFunction.SystemType, "InStr", sqlFunction.Parameters[1],
                                    sqlFunction.Parameters[0], sqlFunction.Parameters[2]);
                        case "AddYear":
                            return new SqlFunction(sqlFunction.SystemType, "Add_Months", sqlFunction.Parameters[0],
                                Mul(sqlFunction.Parameters[1], 12));
                        case "AddQuarter":
                            return new SqlFunction(sqlFunction.SystemType, "Add_Months", sqlFunction.Parameters[0],
                                Mul(sqlFunction.Parameters[1], 3));
                        case "AddMonth":
                            return new SqlFunction(sqlFunction.SystemType, "Add_Months", sqlFunction.Parameters[0],
                        ...
                        case "AddWeekDay":
                        case "AddDay":
                            return Add<DateTime>(sqlFunction.Parameters[0], sqlFunction.Parameters[1]);
                        case "AddWeek":
                            return Add<DateTime>(sqlFunction.Parameters[0], Mul(sqlFunction.Parameters[1], 7));
                        case "AddHour":
                            return Add<DateTime>(sqlFunction.Parameters[0], Div(sqlFunction.Parameters[1], 24));
                        case "AddMinute":
                            return Add<DateTime>(sqlFunction.Parameters[0], Div(sqlFunction.Parameters[1], 60*24));
                        case "AddSecond":
                            return Add<DateTime>(sqlFunction.Parameters[0], Div(sqlFunction.Parameters[1], 60*60*24));
                        case "AddMillisecond":
                            return Add<DateTime>(sqlFunction.Parameters[0],
                                Div(sqlFunction.Parameters[1], 1000*60*60*24));
                        case "Avg":
                ...
                else
                {
                    ...
                    {
                        if (sqlExpression.Expr.StartsWith("To_Number(To_Char(") &&
                            sqlExpression.Expr.EndsWith(", 'FF'))"))
                            return
                                Div(
                                    new SqlExpression(sqlExpression.SystemType,
                                        sqlExpression.Expr.Replace("To_Number(To_Char(", "to_Number(To_Char("),
                                        sqlExpression.Parameters), 1000);
                    }
                }
            }
