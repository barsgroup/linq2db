...
{
    internal class OracleSqlBuilder : BasicSqlBuilder
    {
        private ISqlField _identityField;
        ...
        protected override bool BuildWhere()
        {
            return base.BuildWhere() ||
                   !NeedSkip && NeedTake && SelectQuery.OrderBy.IsEmpty && SelectQuery.Having.IsEmpty;
        }
        ...
        protected override void BuildColumnExpression(IColumn col, ref bool addAlias)
        {
            ...
            if (expr.SystemType == typeof(bool))
            {
                ...
                else
                {
                    ...
                    wrap = ex != null && ex.Expr == "{0}" && ex.Parameters.Length == 1 &&
                           ex.Parameters[0] is ISearchCondition;
                }
            }
        ...
        protected override void BuildCommand(int commandNumber)
        {
            ...
            else
            {
                ...
                {
                    StringBuilder
                        .AppendFormat("CREATE OR REPLACE TRIGGER  TIDENTITY_{0}",
                            SelectQuery.CreateTable.Table.PhysicalName)
                        .AppendLine()
                        .AppendFormat("BEFORE INSERT ON {0} FOR EACH ROW", SelectQuery.CreateTable.Table.PhysicalName)
                        .AppendLine()
                        .AppendLine("BEGIN")
                        .AppendFormat("\tSELECT SIDENTITY_{1}.NEXTVAL INTO :NEW.{0} FROM dual;",
                            _identityField.PhysicalName, SelectQuery.CreateTable.Table.PhysicalName)
                        .AppendLine()
                        .AppendLine("END;");
                }
            }
